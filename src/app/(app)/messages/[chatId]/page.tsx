...
import { 
  PaperAirplaneIcon, ChevronLeftIcon, ArrowPathIcon, UserCircleIcon, PaperClipIcon, XCircleIcon, EyeSlashIcon, EllipsisVerticalIcon, PhoneIcon, VideoCameraIcon,
  ShieldCheckIcon, CheckCircleIcon, ChatBubbleLeftRightIcon, CheckIcon, PhotoIcon, MicrophoneIcon, ChatBubbleLeftEllipsisIcon, TrashIcon,
  ChatBubbleOvalLeftEllipsisIcon, CheckBadgeIcon, CheckIcon as HeroCheckIcon
...
const VerificationBadge = ({ role, isVerified }: { role: UserRoleType | null, isVerified: boolean }) => {
  if (role === 'admin') {
    return <ShieldCheckIcon className="ml-1.5 h-4 w-4 text-amber-400 fill-amber-500 shrink-0" aria-label="Admin" />;
  }
  if (isVerified) {
...
export default function ChatPage() {
  ...
  const otherParticipantInitial = otherParticipantFirstName ? otherParticipantFirstName.charAt(0).toUpperCase() : (otherParticipant?.uid ? otherParticipant.uid.charAt(0).toUpperCase() : <UserCircleIcon className="h-5 w-5" />);
      ...
      >
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
            <ChatBubbleOvalLeftEllipsisIcon className="h-12 w-12 mb-2 opacity-30" />
                        ...
                        <div className={cn(
                          ...
                          {isSender && (
                            <span className="ml-0.5">
                              {msg.status === 'read' ? (
                                <CheckBadgeIcon className="h-3 w-3 text-blue-400" />
                              ) : (
                                <HeroCheckIcon className="h-3 w-3 opacity-50" />
                              )}
                            </span>
                          )}
                        </div>
              ...
              <div className="relative">
                ...
                >
                  {sendingMessage ? (
                    ...
                    <PaperAirplaneIcon className="h-5 w-5 text-white" /> // Send icon when voice message is ready
                  ) : newMessage.trim() || selectedFile ? (
