...
import { UserSearchResultCard } from './UserSearchResultCard'; // Import the new component
        ...
        {maxDiscount > 0 && (
          <div className="absolute top-2 left-2 z-10">
            <Badge variant="secondary" className="bg-green-500/90 text-white">
              <Percent className="h-3 w-3 mr-1" /> Up to {maxDiscount}% off
            </Badge>
          </div>
        )}
        ...
        <div className="p-4">
          ...
          <div className="flex items-center gap-3 mt-3 text-sm text-muted-foreground">
            {plan.participantsCount !== undefined && (
              <div className="flex items-center gap-1"> <Users className="h-4 w-4" /> {plan.participantsCount} </div>
            )}
            {plan.likesCount !== undefined && (
              <div className="flex items-center gap-1"> <Heart className="h-4 w-4" /> {plan.likesCount} </div>
            )}
            {plan.sharesCount !== undefined && (
              <div className="flex items-center gap-1"> <Share2 className="h-4 w-4" /> {plan.sharesCount} </div>
            )}
            {plan.savesCount !== undefined && (
              <div className="flex items-center gap-1"> <BookmarkPlus className="h-4 w-4" /> {plan.savesCount} </div>
            )}
          </div>
          <div className="flex items-center gap-2 mt-3">
            <Avatar className="h-6 w-6">
              <AvatarImage src={plan.creatorAvatarUrl} />
              <AvatarFallback>{plan.creatorName?.[0]?.toUpperCase() || 'U'}</AvatarFallback>
            </Avatar>
            <div className="flex items-center gap-1">
              <span className="text-sm font-medium">{plan.creatorName}</span>
              {plan.creatorIsVerified && (<BadgeCheck className="h-4 w-4 text-primary" />)}
            </div>
          </div>
          <div className="flex items-center gap-2 mt-4">
            <Button variant="outline" size="sm" className="flex-1" onClick={handleSave} disabled={saving}>
              {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <><BookmarkPlus className="h-4 w-4 mr-2" /> Add to My Macaroon</>}
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="group relative h-[500px] rounded-2xl overflow-hidden bg-black/90">
        ...
        {isAdmin && onRemoveFeature && (
          <div className="absolute top-4 right-4 z-10">
            <Button variant="destructive" size="sm" onClick={(e) => { e.preventDefault(); onRemoveFeature(); }}> Remove Featured </Button>
          </div>
        )}
        <div className="absolute top-4 left-4 flex flex-col gap-2">
          <Badge variant="premium" className="bg-gradient-to-r from-amber-500 to-amber-700"> <Crown className="h-3 w-3 mr-1" /> Featured </Badge>
          {plan.isPremiumOnly && (<Badge variant="premium" className="bg-gradient-to-r from-amber-500 to-amber-700"> <Lock className="h-3 w-3 mr-1" /> Premium </Badge>)}
          {maxDiscount > 0 && (<Badge variant="secondary" className="bg-green-500/90 text-white"> <Percent className="h-3 w-3 mr-1" /> Up to {maxDiscount}% off </Badge>)}
        </div>
        <div className="absolute inset-x-0 bottom-0 p-6">
          <div className="max-w-2xl">
            ...
            <div className="flex items-center gap-6 text-white/80">
              {plan.participantsCount !== undefined && (<div className="flex items-center gap-2"> <Users className="h-5 w-5" /> {plan.participantsCount} </div>)}
              {plan.likesCount !== undefined && (<div className="flex items-center gap-2"> <Heart className="h-5 w-5" /> {plan.likesCount} </div>)}
              {plan.averageRating && (<div className="flex items-center gap-2 text-amber-400"> <Star className="h-5 w-5 fill-current" /> <span className="font-medium">{plan.averageRating.toFixed(1)}</span> </div>)}
            </div>
            <div className="flex items-center gap-3 mt-6">
              <Avatar className="h-10 w-10 ring-2 ring-white/20">
                <AvatarImage src={plan.creatorAvatarUrl} />
                <AvatarFallback className="bg-primary/20 text-white">{plan.creatorName?.[0]?.toUpperCase() || 'U'}</AvatarFallback>
              </Avatar>
              <div className="flex items-center gap-2">
                <span className="text-lg font-medium text-white">{plan.creatorName}</span>
                {plan.creatorIsVerified && (<BadgeCheck className="h-5 w-5 text-blue-400" />)}
              </div>
            </div>
          </div>
        </div>
...
export function ExploreContent({ initialData, userPreferences }: ExploreContentProps) {
  ...
  const categoryImages: Record<string, string> = {
    'ALL': '/images/categories/all.jpg', 'ART': '/images/categories/art.jpg', 'FITNESS': '/images/categories/fitness.jpg',
  };
  ...
  const requestLocation = useCallback(() => {
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          const coords: GeoPoint = { latitude: position.coords.latitude, longitude: position.coords.longitude };
          try {
            ...
            if (data.results?.[0]?.address_components) {
              ...
              if (city && country) setUserLocation({ city, country, coordinates: coords });
              else throw new Error('Could not find city and country in geocoding results');
            } else throw new Error('No address components found in geocoding results');
          } catch (error: any) {
    ...
    if (user?.uid) {
      getUserLocationAction(user.uid).then(result => {
        if (result.success && result.data?.city && result.data?.country) {
          ...
          toast({ title: 'Enable Location', description: 'Enable location access for personalized recommendations.', action: <Button onClick={requestLocation}>Enable</Button>, duration: 0 });
        }
      }).catch(err => {
        if (!locationRequested) {
           toast({ title: 'Location Unavailable', description: 'Could not retrieve saved location. Want to enable it now?', action: <Button onClick={requestLocation}>Enable</Button>, duration: 0 });
        }
      });
    }
  ...
  const processExploreData = useCallback((allPlansData: Plan[], featuredCitiesData: City[], currentLocation?: { city: string; country: string; coordinates?: GeoPoint }) => {
    ...
    const citiesWithScore = featuredCitiesData.map(city => ({
      ...city,
      score: scoredPlans.filter(p => p.city?.toLowerCase() === city.name?.toLowerCase()).reduce((acc, p) => acc + (p as any).score, 0)
    })).sort((a, b) => b.score - a.score);
        ...
        <div className="max-w-screen-2xl mx-auto w-full">
          <div className="px-4 py-6 relative">
            {isSearchActive ? (
              <div className="mb-6">
                {searchLoading && (
                  <div className="flex justify-center items-center py-10 min-h-[200px]"> <Loader2 className="h-8 w-8 animate-spin text-primary" /> </div>
                )}
                {!searchLoading && filteredPlans.length === 0 && userSearchResults.length === 0 && (
                  <div className="text-center py-8"><p className="text-muted-foreground">No results found for "{searchTerm}"</p></div>
                )}
                {userSearchResults.length > 0 && (
                  <Section title="Matching People" className="mb-8">
                ...
                {filteredPlans.length > 0 && (
                  <Section title="Matching Plans" className="mb-8">
              ...
              <>
                ...
                {filteredPlans.length > 0 && (
                  <Section title="Popular Plans" viewAllHref="/plans" className="space-y-6">
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {filteredPlans.slice(0, 6).map(plan => ( <PlanCard key={plan.id} plan={plan} /> ))}
                    </div>
                  </Section>
                )}
                 {!loading && plans.length === 0 && featuredPlans.length === 0 && !isSearchActive && (
                    <div className="text-center py-12 text-muted-foreground">
                        <Search className="mx-auto h-16 w-16 opacity-30 mb-3" />
                        <p className="font-semibold text-lg">No plans to show right now.</p>
                        <p className="text-sm">Try adjusting your location or search terms, or check back later!</p>
                    </div>
                )}
              </>
            ) : (
              <div className="text-center py-10">
                <p className="text-muted-foreground">Select a view (All, Cities, Categories, etc.) to see content.</p>
              </div>
            )}
          </div>
        </div>
