...
import { COMMON_ACTIVITY_TYPES } from "@/lib/constants";

interface ActivityTypePreferencesInputProps {
  value: string[];
  onChange: (value: string[]) => void;
}

export function ActivityTypePreferencesInput({ value: selectedPreferences, onChange }: ActivityTypePreferencesInputProps) {
  const [open, setOpen] = useState(false);
  const [inputValue, setInputValue] = useState("");
  const [allOptions, setAllOptions] = useState<string[]>(() =>
    [...new Set([...COMMON_ACTIVITY_TYPES, ...selectedPreferences])].sort()
  );
  const inputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    const newOptions = [...new Set([...COMMON_ACTIVITY_TYPES, ...selectedPreferences, ...allOptions])].sort();
  ...
  return (
    <div className="space-y-2">
      <Popover open={open} onOpenChange={setOpen}>
          ...
          >
            <div className="flex flex-wrap gap-1 items-center">
              {selectedPreferences.length > 0 ? (
                ...
                <span className="text-muted-foreground font-normal">Select activity preferences...</span>
              )}
            </div>
        ...
        <PopoverContent className="w-[--radix-popover-trigger-width] p-0 dropdown-content-width-same-as-trigger">
          <Command>
            <CommandInput
              ref={inputRef}
              placeholder="Search or add activity type..."
              value={inputValue}
      ...
      <p className="text-xs text-muted-foreground px-1">
        What types of activities do you generally enjoy?
      </p>
    </div>
  );
}
