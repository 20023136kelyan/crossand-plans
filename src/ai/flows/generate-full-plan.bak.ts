...
import type { Plan, PriceRangeType, PlanType, ItineraryItem } from '@/types/user';
...
import { z } from 'zod';
import { UserProfile, AppTimestamp } from '../../types/user';
...
export async function generateFullPlan(input: z.infer<typeof GeneratePlanInputSchema>): Promise<Plan> {
  try {
    ...
    const duration = "2 hours"; // Default duration
    const groupSize = String(input.invitedFriendProfiles.length + 1); // Host + friends
    const activityType = "general";
    const timeOfDay = new Date(input.planDateTime).getHours() < 12 ? "morning" : 
                      new Date(input.planDateTime).getHours() < 18 ? "afternoon" : "evening";
    const budget = input.priceRange || "moderate";
    ...
    // Define the prompt template using the ai.definePrompt function
...
`,
    ...
    // Create a complete UserProfile from the hostProfile with all required fields
    const userProfile: UserProfile = {
      ...
      // Copy preferences from AIEnhancedProfileSchema
      ...
      // Timestamps
      createdAt: new Date().toISOString() as AppTimestamp,
